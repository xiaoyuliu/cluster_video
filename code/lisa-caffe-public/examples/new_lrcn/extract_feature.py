...
model_root    = os.path.join( data_root, 'output/UCF-101/snapshots_lstm_RGB' )
...
def convert_feature(out_filename, N, F, device_id, batch_size, mode=0):
  ...
  net = caffe.Net( os.path.join( net_root, 'train_test_lstm_RGB.prototxt'  ),
                 os.path.join( model_root, '_iter_2500.caffemodel'),
                 caffe.TEST)
  for batch_num in range( np.ceil( float(N)/batch_size ) ):
    net.forward()
    pdb.set_trace()
